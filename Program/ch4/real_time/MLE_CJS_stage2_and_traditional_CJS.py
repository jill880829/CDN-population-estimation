...
for i in range(3, 11):
    ...
    capturing_p[i-1] = cp_before_sort[-1][1]

whole_estimation = np.array(whole_sample[2:]) / capturing_p[2:]
print("estimation")
print(whole_estimation)
err = np.mean(np.abs( whole_estimation[:-1] - server_num_each_day[2:-1] ) / server_num_each_day[2:-1])
print("error")
print(err)
...
print("error")
print(np.mean(np.abs(estimate_N_24hr_us[2:-1] - server_num_each_day[2:-1]) / server_num_each_day[2:-1]))
...
y_axis2 = estimate_N_24hr_us[2:-1]
...
plt.savefig(argv[3]+'real_time.png')